...
class ModeloVentas{
	...
	static function newIdIncrement()
	{
		$stmt = Conexion::conectar()->prepare("SELECT * FROM compras ORDER BY id DESC");
		...
		$ultimoIdPedido = $resultado['id_pedido'];

		$ultimoIdPedido = explode('-', $ultimoIdPedido);

		$numeroIncrementable = (int) $ultimoIdPedido[1];

		$numeroIncrementable += 1;

		return "$mes-$numeroIncrementable-$anio";
	}
