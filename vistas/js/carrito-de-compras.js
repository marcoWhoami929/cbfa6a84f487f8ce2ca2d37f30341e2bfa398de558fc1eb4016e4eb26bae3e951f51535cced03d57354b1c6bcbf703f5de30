...
$(".agregarCarrito").click(function(){
	...
	var existencias = $(this).attr("existencias");
	var cantidad= $("#seleccionarCantidad").val();
	console.log(cantidad);


	/*=============================================
	VALIDAR QUE EL CAMPO ESTE SELECCIONADO
	=============================================*/
	if ($("#seleccionarCantidad option:selected").val()==0) {
				swal({
					  type: 'error',
					  title: 'Oops...',
					  text: 'No ha seleccionado una cantidad!',
	...
	/*==============================================
	VALIDAR QUE LA CANTIDAD SEA MENOR A LA EXISTENCIA
	=============================================*/
	if($("#seleccionarCantidad option:selected").val() > $(this).attr("existencias")){
		swal({
					  type: 'error',
					  title: 'Oops...',
					  text: 'Debe elegir una cantidad menor a las existencias!',
			...
			for (var i = 0; i < listaProductos.length; i++) {
				...
				if (listaProductos[i]["idProducto"] == idProducto) {
						...
						swal({
							  ...
							  text: "Modifique la cantidad en el carrito de compras",
							  type: "warning",
...
$(".cantidadItem").change(function(){
	...
	var precioInput = parseInt($(this).attr("precio"));
	var idProducto = $(this).attr("idProducto");
	...
	$.ajax({
		...
		success: function (respuesta){
			...
			var max = parseInt(respuesta.stock) + parseInt(respuesta.agregarStock);

			if (cantidadInput > max) {
				input.val(cantidadInput - 1);
				return;
			}

			$(".subTotal"+item).html('<strong>MXN $<span>'+(cantidadInput*precioInput)+'</span></strong>');
	...
	for(var i = 0; i < titulo.length; i++){
		...
		if(tipoArray.find(checkTipo) == "fisico"){
			...
			$("#seleccionarPais").change(function(){
				...
				/*=============================================
				RETORNAR CAMBIO DE DIVISA A MXN
				=============================================*/
				...
				$(".valorSubtotal").html(1 * Math.floor(Number($(".valorSubtotal").attr("valor"))*100)/100)
		    	$(".valorTotalEnvio").html(1 * Math.floor(Number($(".valorTotalEnvio").attr("valor"))*100)/100)
		    	$(".valorTotalImpuesto").html(1 * Math.floor(Number($(".valorTotalImpuesto").attr("valor"))*100)/100)
		    	$(".valorTotalCompra").html(1 * Math.floor(Number($(".valorTotalCompra").attr("valor"))*100)/100)

		    	var valorItem = $(".valorItem");

		    	for(var i = 0; i < valorItem.length; i++){

		    		$(valorItem[i]).html(1 * Math.floor(Number($(valorItem[i]).attr("valor"))*100)/100)
