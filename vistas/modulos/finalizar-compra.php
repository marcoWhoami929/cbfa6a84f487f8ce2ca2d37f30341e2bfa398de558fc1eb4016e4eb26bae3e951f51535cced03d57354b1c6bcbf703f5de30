...
$id_pedido = ModeloVentas::newIdIncrement();
...
if(isset( $_GET['paypal']) && $_GET['paypal'] === 'true'){
   ...
   for($i = 0; $i < count($productos); $i++){
         ...
         $producto = ModeloProductos::mdlMostrarInfoProducto("productos", "id", $productos[$i]);
   		...
   		if($respuesta == "ok"){
            ...
            foreach ($_SESSION["productos"] as $productoCompra) {
               $nuevaExistencia = $producto['existencias'] - $productoCompra['cantidad'];

               $actualizarCompra = ControladorProductos::ctrActualizarProducto('existencias', $nuevaExistencia, 'id', $productoCompra['idProducto']);
            }
